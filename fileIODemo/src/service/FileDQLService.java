...
public interface FileDQLService {
     ...
     * @return 对应集装箱 的 （最长）服务时间
     */
    Result<Map<String, Integer>> getTimeServedBy(String path, Container container);
     ...
     * @param company
     * @param city
     * @return 返回 <"courier.name:courier.phone",total_num> 的映射表
     */
    Result<Map<String, Integer>> getCourier(String path, Company company, City city);

    /**
     * 找到 城市 使得 总出口成本最低 (?)
     *
     * @param path
     * @return 返回 <"city.name",total_cost> 的映射表
     */
    Result<Map<String, Double>> getCity(String path);
